...
import { actualizarProductos, crearProducto } from "../../../../Services/ApiProductos";
...
const Formulario = ({data, nuevo}) => {
  ...
  const {
    ...
    const handleChange = (e) => {
      ...
      // if (e.target.name === 'imagen' && e.target.files.length > 0) {
    ...
    <div className="flex gap-2 w-full justify-center p-2">
     <div className=" text-base ">
      ...
      <div id="imagen" 
      className="flex flex-col items-start p-2 rounded-lg w-56 h-56" 
      style={{
        backgroundImage: `url(${imagenPreview})` , backgroundSize: 'cover',  backgroundPosition: 'center'
      }}
      >
      <label 
      className="text-sm font-semibold bg-green-400  p-1 rounded-xl" 
      >
      ...
      <button className="btn border-2 hover:bg-lime-300" type="submit">Guardar</button>
      </div>
      </form>
     </div>
